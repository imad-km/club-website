...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($act === 'invite') {
        ...
        if (!$pl) { $flash = 'Invalid request.'; $flashType = 'err'; }
            ...
            if(!$em){$flash='Email required.';$flashType='err';}
                ...
                $r=api('professor/invite','POST',['name'=>$nm,'email'=>$em]);
    ...
    if ($act === 'ban_student') {
        ...
        if (!$pl) { $flash = 'Invalid request.'; $flashType = 'err'; }
            ...
            if (!$identify || !$reason) { $flash='Identify and reason are required.'; $flashType='err'; }
                ...
                $r = api('professor/ban','POST',['identify'=>$identify,'reason'=>$reason]);
    ...
    if ($act === 'unban_student') {
        ...
        if (!$pl) { $flash = 'Invalid request.'; $flashType = 'err'; }
            ...
            if (!$identify) { $flash='Identify is required.'; $flashType='err'; }
                ...
                $r = api('professor/unban','POST',['identify'=>$identify]);
    ...
    header('Location: dashboard.php' . (isset($_GET['pg']) ? '?pg='.urlencode($_GET['pg']) : '')); exit();
}
...
$endpoints = ['me' => 'me'];
if ($pg === 'students')  $endpoints['students'] = 'professor/fetch-all';
...
.badge-licence{background:#f0fdf4;color:#15803d;border:1px solid #bbf7d0;}
.badge-master{background:#eff6ff;color:#2563eb;border:1px solid #bfdbfe;}
.badge-doctorat{background:var(--pl);color:var(--purple);border:1px solid rgba(124,58,237,.2);}
...
<nav class="sb" id="sb">
  ...
  <div class="sb-nav">
    ...
    <a href="?pg=students" class="nav-item <?=$pg==='students'?'active':''?>">
      <svg viewBox="0 0 24 24"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg>
      Students
      <span class="nbadge"><?=count($students)?></span>
    </a>
  ...
  <div class="sb-foot">
    ...
    <div class="sb-prof">
      ...
      <?php endif; ?>
      <div><div class="sb-prof-name"><?=htmlspecialchars($fullName)?></div><div class="sb-prof-role">Professor</div></div>
    </div>
...
<div class="main">
  <header class="topbar">
    <div style="display:flex;align-items:center;gap:12px;">
      ...
      <div>
        <?php
          $titles = ['students'=>'Students','annonces'=>'Announcements','events'=>'Events','settings'=>'Settings'];
          $subs   = ['students'=>'Enrolled faculty members','annonces'=>'Posts for your students','events'=>'Upcoming & past events','settings'=>'Manage your account'];
        ?>
    ...
    <div class="tb-right">
      ...
      <div class="search">
        ...
        <input type="text" id="q" placeholder="Search students…" oninput="filterStu(this.value)"/>
      </div>
  ...
  <div class="content">
    ...
    <?php if($pg==='students'): ?>
    <div class="stats">
      <div class="stat"><div class="stat-icon g"><svg viewBox="0 0 24 24"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/></svg></div><div class="stat-val"><?=count($students)?></div><div class="stat-lbl">Total Students</div></div>
      <?php
        $gradeCount  = array_count_values(array_column($students,'grade'));
        $domCount    = array_count_values(array_column($students,'domain'));
        $topDomain   = $domCount ? array_search(max($domCount),$domCount) : '—';
        $bannedCount = count(array_filter($students, fn($s)=>!empty($s['is_banned'])));
      ?>
      <div class="stat"><div class="stat-icon b"><svg viewBox="0 0 24 24"><path d="M22 10v6M2 10l10-5 10 5-10 5z"/><path d="M6 12v5c3 3 9 3 12 0v-5"/></svg></div><div class="stat-val"><?=$gradeCount['master']??0?></div><div class="stat-lbl">Master</div></div>
      <div class="stat"><div class="stat-icon p"><svg viewBox="0 0 24 24"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg></div><div class="stat-val"><?=$gradeCount['licence']??0?></div><div class="stat-lbl">Licence</div></div>
      <div class="stat"><div class="stat-icon a"><svg viewBox="0 0 24 24"><path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/></svg></div><div class="stat-val"><?=$gradeCount['doctorat']??0?></div><div class="stat-lbl">Doctorat</div></div>
    ...
    <div class="sec-bar">
      <div><div class="sec-title">All Students</div><div class="sec-hint" id="stuHint"><?=count($students)?> student<?=count($students)!==1?'s':''?> enrolled</div></div>
      <div class="chips">
        <button class="chip on" onclick="setChip('all',this)">All</button>
        <button class="chip" onclick="setChip('licence',this)">Licence</button>
        <button class="chip" onclick="setChip('master',this)">Master</button>
        <button class="chip" onclick="setChip('doctorat',this)">Doctorat</button>
    ...
    <div class="tbl-wrap">
      ...
      <div class="empty-state"><svg viewBox="0 0 24 24"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/></svg><p>No students found</p></div>
      <?php else: ?>
      <table id="stuTbl">
        ...
        <tbody id="stuBody">
          <?php foreach($students as $i=>$s):
            ...
            $gradeClass = 'badge-'.strtolower($grade);
...
<div class="modal-bg hidden" id="annModal">
  <div class="modal" style="max-width:480px;">
    ...
    <p>Publish a message visible to all students.</p>
    <form method="POST" action="?pg=annonces" id="annForm">
      ...
      <div class="toggle-row"><label>Visible to students</label><label class="tog"><input type="checkbox" id="ann_visible" checked/><span class="tog-sl"></span></label></div>
      <div class="modal-actions">
...
<div class="modal-bg hidden" id="evtModal">
  <div class="modal" style="max-width:480px;">
    <h3>New Event</h3>
    <p>Schedule an event for your students.</p>
    <form method="POST" action="?pg=events" id="evtForm">
      ...
      <div class="toggle-row"><label>Visible to students</label><label class="tog"><input type="checkbox" id="evt_visible" checked/><span class="tog-sl"></span></label></div>
      <div class="modal-actions">
...
/* ── Student filter ── */
var ALL_STUDENTS = <?=json_encode(array_map(fn($s)=>[
  'fn'        => $s['firstname']??'',
  'ln'        => $s['lastname']??'',
  'email'     => $s['email']??'',
  'phone'     => $s['phone']??'',
...
var GRADE_CLASS = {licence:'badge-licence',master:'badge-master',doctorat:'badge-doctorat'};
...
function filterStu(q){
  ...
  tb.innerHTML='';
  if(!list.length){hint.textContent='No students found';return;}
