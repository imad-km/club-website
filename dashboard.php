...
.drawer-translate-label{
  font-size:.62rem;font-weight:700;letter-spacing:.06em;text-transform:uppercase;
  color:var(--muted);display:flex;align-items:center;gap:4px;white-space:nowrap;
}
.drawer-translate-label i{color:var(--green);font-size:.65rem;}
.drawer-lang-btns{display:flex;gap:4px;}
.drawer-lang-btn{
  ...
  letter-spacing:.07em;cursor:pointer;transition:.18s;
}
.drawer-lang-btn:hover{border-color:var(--green);color:var(--green);background:var(--green-p);}
.drawer-lang-btn.active{background:var(--green);color:#fff;border-color:var(--green);box-shadow:0 2px 8px rgba(27,110,63,.25);}
</style>
</head>
<body>
<nav class="top-nav">
...
<div class="drawer" id="post-drawer">
  <div class="drawer-head">
    ...
    <div class="drawer-title" id="drawer-title" style="flex:1">Actualité</div>
    <div class="drawer-translate-wrap">
      <span class="drawer-translate-label"><i class="fa-solid fa-wand-magic-sparkles"></i> Translate with AI !</span>
      <div class="drawer-lang-btns">
        <button class="drawer-lang-btn active" data-lang="ar" onclick="setDrawerLang('ar',this)">AR</button>
        <button class="drawer-lang-btn" data-lang="fr" onclick="setDrawerLang('fr',this)">FR</button>
        <button class="drawer-lang-btn" data-lang="en" onclick="setDrawerLang('en',this)">EN</button>
      </div>
    </div>
  </div>
...
function timeAgo(iso){const d=(Date.now()-new Date(iso))/1000;if(d<60)return`à l'instant`;if(d<3600)return`il y a ${Math.floor(d/60)} min`;if(d<86400)return`il y a ${Math.floor(d/3600)} h`;if(d<172800)return'hier';return`il y a ${Math.floor(d/86400)} jours`;}
...
function buildProjectCard(p,delay){
  ...
  const imgSection=p.image
    ?`<div class="pc-img-wrap"><img class="pc-img lb-trigger" src="${p.image}" alt="" loading="lazy" onclick="openLightbox('${esc(p.image)}','${esc(p.title)}');" onerror="this.closest('.pc-img-wrap').style.display='none'"></div>`
    :'';
...
function buildEventCard(e,delay){
  ...
  const imgContent=e.image?`<img src="${e.image}" alt="" class="lb-trigger" onclick="openLightbox(this.src,'${esc(e.title)}')">`:
...
function setDrawerLang(lang,btn){
  drawerLang=lang;
  document.querySelectorAll('.drawer-lang-btn').forEach(b=>b.classList.toggle('active',b.dataset.lang===lang));
  if(drawerCurrentId!==null) reloadDrawerContent(drawerCurrentId);
}

async function loadUniversityInfo(){
  if(uniLoaded)return;
  document.getElementById('uni-loading').style.display='block';
  document.getElementById('uni-posts-list').innerHTML='';
  try{
    const data=await _postAjax({_action:'get_uni_posts'});
    uniLoaded=true;uniPosts=data.posts||[];
    document.getElementById('uni-loading').style.display='none';
    if(!uniPosts.length){document.getElementById('uni-posts-list').innerHTML=`<div class="empty-state"><div class="es-icon"><i class="fa-solid fa-university"></i></div><div class="es-title">Aucune actualité</div></div>`;return;}
    document.getElementById('uni-posts-list').innerHTML=uniPosts.map((p,i)=>{
...
async function reloadDrawerContent(id){
  ...
  try{
    ...
    document.getElementById('drawer-content').innerHTML=`<div class="empty-state"><div class="es-icon"><i class="fa-solid fa-wifi"></i></div><div class="es-title">Erreur de chargement</div></div>`;
  }
}
...
function renderDrawerContent(data){
  document.getElementById('drawer-loading').style.display='none';
  document.getElementById('drawer-content').style.display='block';
  const isRtl=drawerLang==='ar';
  const dir=isRtl?' dir="rtl"':'';
  let html=`<div${dir}>`;
  if(data.text)html+=`<div class="drawer-text">${data.text}</div>`;
  if(data.images?.length){html+=`<div class="drawer-imgs-title"><i class="fa-solid fa-images" style="color:var(--green)"></i> Photos (${data.images.length})</div><div class="drawer-img-grid">`;data.images.forEach(src=>{html+=`<img src="${src}" alt="" loading="lazy" class="lb-trigger" onclick="openLightbox(this.src,'')" onerror="this.style.display='none'">`});html+=`</div>`;}
  if(data.pdf?.length){data.pdf.forEach((url,j)=>{const name=url.split('/').pop()||`document-${j+1}.pdf`;html+=`<div class="drawer-pdf-viewer"><div class="drawer-pdf-viewer-header"><div style="display:flex;align-items:center;gap:8px"><div class="drawer-pdf-icon"><i class="fa-solid fa-file-pdf"></i></div><div style="font-size:.82rem;font-weight:700;color:var(--text)">${name}</div></div><a href="${url}" target="_blank" rel="noopener" class="drawer-pdf-open-btn"><i class="fa-solid fa-arrow-up-right-from-square"></i> Ouvrir</a></div><iframe src="${url}" class="drawer-pdf-iframe" title="${name}"></iframe></div>`;});}
  if(!data.text&&!data.images?.length&&!data.pdf?.length)html+=`<div class="empty-state"><div class="es-icon"><i class="fa-solid fa-file-circle-question"></i></div><div class="es-title">Contenu indisponible</div></div>`;
  html+=`</div>`;
  document.getElementById('drawer-content').innerHTML=html;
}

async function openPostDrawer(id,title){
  drawerCurrentId=id;
  drawerLang='ar';
  document.getElementById('drawer-title').textContent=title||'Actualité';
  document.getElementById('drawer-content').style.display='none';
  document.getElementById('drawer-loading').style.display='flex';
  document.getElementById('drawer-overlay').classList.add('open');
  document.getElementById('post-drawer').classList.add('open');
  document.body.style.overflow='hidden';
  document.querySelectorAll('.drawer-lang-btn').forEach(b=>b.classList.toggle('active',b.dataset.lang==='ar'));
  try{
    const data=await _postAjax({_action:'get_uni_post',id});
    renderDrawerContent(data);
  }catch(e){
    document.getElementById('drawer-loading').style.display='none';
    document.getElementById('drawer-content').style.display='block';
